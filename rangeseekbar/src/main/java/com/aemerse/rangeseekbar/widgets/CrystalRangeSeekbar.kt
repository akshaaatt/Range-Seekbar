...
open class CrystalRangeSeekbar @JvmOverloads constructor(
    ...
    var thumbWidthHere = 0f
    private var thumbDiameter = 0f
    var thumbHeightHere = 0f
    ...
    protected open fun init() {
        ...
        thumbWidthHere = getThumbWidth()
        thumbHeightHere = getThumbHeight()

        //thumbHalfWidth = thumbWidthHere / 2;
        //thumbHalfHeight = thumbHeightHere / 2;
    ...
    fun apply() {
        ...
        //thumbHalfHeight = thumbHeightHere / 2;
        barHeight = getBarHeight()
        barPadding = thumbWidthHere * 0.5f
    ...
    protected fun getBarHeight(): Float {
        return if (_barHeight > 0) _barHeight else thumbHeightHere * 0.5f * 0.3f
    }

    protected fun getBarPadding(): Float {
        return thumbWidthHere * 0.5f
    }
    ...
    protected fun getBarColorMode(typedArray: TypedArray): Int {
        return typedArray.getInt(R.styleable.CrystalRangeSeekbar_bar_color_mode,
    ...
    protected fun getBarHighlightColorMode(typedArray: TypedArray): Int {
        return typedArray.getInt(R.styleable.CrystalRangeSeekbar_bar_highlight_color_mode,
            CrystalSeekbar.ColorMode.SOLID)
    }
    ...
    protected fun getBarHighlightGradientStart(typedArray: TypedArray): Int {
        return typedArray.getColor(R.styleable.CrystalRangeSeekbar_bar_highlight_gradient_start,
            Color.DKGRAY)
    }

    protected fun getBarHighlightGradientEnd(typedArray: TypedArray): Int {
        return typedArray.getColor(R.styleable.CrystalRangeSeekbar_bar_highlight_gradient_end,
            Color.BLACK)
    }
    ...
    protected open fun getLeftThumbColorPressed(typedArray: TypedArray): Int {
        return typedArray.getColor(R.styleable.CrystalRangeSeekbar_left_thumb_color_pressed,
            Color.DKGRAY)
    }

    protected open fun getRightThumbColorPressed(typedArray: TypedArray): Int {
        return typedArray.getColor(R.styleable.CrystalRangeSeekbar_right_thumb_color_pressed,
            Color.DKGRAY)
    }
    ...
    protected fun getDiameter(typedArray: TypedArray): Float {
        return typedArray.getDimensionPixelSize(R.styleable.CrystalRangeSeekbar_thumb_diameter,
            resources.getDimensionPixelSize(R.dimen.thumb_height)).toFloat()
    }
    ...
    protected fun setupLeftThumb(canvas: Canvas, paint: Paint?, rect: RectF?) {
        ...
        leftThumbRect!!.bottom = thumbHeightHere
        if (leftThumb != null) {
    ...
    protected open fun drawLeftThumbWithColor(canvas: Canvas, paint: Paint?, rect: RectF?) {
    ...
    protected fun setupRightThumb(canvas: Canvas, paint: Paint?, rect: RectF?) {
        ...
        rightThumbRect!!.bottom = thumbHeightHere
        if (rightThumb != null) {
    ...
    protected open fun drawRightThumbWithColor(canvas: Canvas, paint: Paint?, rect: RectF?) {
        canvas.drawOval(rect!!, paint!!)
    }
    ...
    protected fun getMeasureSpecHeight(heightMeasureSpec: Int): Int {
        var height = Math.round(thumbHeightHere)
    ...
    private fun isInThumbRange(touchX: Float, normalizedThumbValue: Double): Boolean {
        ...
        if (thumbPos > width - thumbWidthHere) x = touchX
        return x >= left && x <= right
        //return Math.abs(touchX - normalizedToScreen(normalizedThumbValue)) <= thumbWidthHere;
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        setMeasuredDimension(getMeasureSpecWith(widthMeasureSpec),
            ...
            MotionEvent.ACTION_UP -> {
                if (mIsDragging) {
                    ...
                    if (onRangeSeekbarFinalValueListener != null) {
                        onRangeSeekbarFinalValueListener!!.finalValue(selectedMinValue,
                            selectedMaxValue)
                    }
                } else {
